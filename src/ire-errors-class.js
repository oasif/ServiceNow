...
while (sysLog.next()) {
    Object.keys(ireErrorMessagesObj).forEach(function (key) {
        var jsonString = sysLog.message.split("'")[1];
        try {
        ...
        if (sysLog.message.indexOf(key) > -1) {
            ...
            ireErrorMessagesObj[key][className] ? ireErrorMessagesObj[key][className]++ : ireErrorMessagesObj[key][className] = 1;
        }
...
//     }
// });
gs.info(JSON.stringify(ireErrorMessagesObj));
//attachCsv2Kb("ire-errors", out);
